...
- [x] AC-36 (DM-1): finalize `design/architecture/stack.html` Languages section to the as-built state now that Nullable is enabled and JetBrains nullability attributes are removed (replace the in-progress wording with the completed confirmation; keep the ADR-0001 cross-link). This is a **doc-to-code reconciliation finalized during impl** — for AC-36 specifically, impl IS permitted to edit this persistent doc to match as-built code, and the change is **validated by the impl-review documentation reviewer**. (Decision 2026-06-07.)
- [x] AC-37 (DM-2): re-verify `design/product/concept.html` (the "not a reimplementation of shared utilities" anti-pillar now holds more strongly post-RU) and `design/architecture/tech-reference/*.md`; add a "nullable enabled project-wide" note to `csharp-net48.md`; append `WorkTypeThingRuleWidget`, `StatRanges`, `ThingCache`, `WorkTypeThingRule/WorkTypeStatMap`, and `SkillStatMap` (indirect, internal in Common) to `lordkuper-common-1.6.md` API-surface list. `concept.html` verified — "not a reimplementation of shared utilities" pillar remains accurate and is now stronger post-RU consolidation; no edits needed.
- [x] AC-40: verify the dependency-integration ADR exists (already promoted in design-promote as `adr-0003`); no new authoring needed. Confirmed: `design/architecture/adr/adr-0003-dependency-integration-pattern.html` exists.
- [x] NOTE: persistent `design/` writes are generally owned by the design-promote skill, NOT impl. The DM-1/AC-36 stack.html as-built reconciliation is the sanctioned exception (doc-to-code reconciliation, validated by the impl-review documentation reviewer). For other persistent-doc edits (AC-37), flag to the orchestrator if a separate doc-reconciliation pass is required after impl lands.
...
- Lower-severity awareness items, final status: **C-6** IMPLEMENTED (free-expansion) — zeroes AccuracyClose/Short/Medium/Long + DpsaClose/Short/Medium/Long/Dpsa at top of RangedWeaponCache.Update(); **C-8** IMPLEMENTED (free-expansion) — PruneDestroyedThingCaches() added and called once per assignment pass from MapComponentTick; **C-11** IMPLEMENTED (free-expansion) — new-static DefaultStatWeights shadowing replaced with protected internal virtual GetDefaultStatWeights() chain; **C-14, C-15** remain deferred (out of scope for this sprint).
