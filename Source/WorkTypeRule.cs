...
internal class WorkTypeRule : IExposable
{
    ...
    private static readonly Dictionary<string, IEnumerable<string>> DefaultWorkTypeStats = new()
    {
        {
            "Cooking", [
                "FoodPoisonChance", "DrugCookingSpeed", "ButcheryFleshSpeed",
                "ButcheryFleshEfficiency",
                "CookSpeed"
            ]
        },
        { "Hunting", ["HuntingStealth"] },
        { "Doctor", ["MedicalTendQualityOffset", "MedicalPotency"] }
    };

    private readonly HashSet<StatDef> _requiredStats = [];
    ...
    [NotNull]
    public IEnumerable<Thing> GetCurrentlyAvailableItemsSorted([CanBeNull] Map map)
    {
    ...
    public IEnumerable<ThingDef> GetGloballyAvailableItems()
    {
        ...
        items.AddRange(AllRelevantThings.Where(def =>
            (def.statBases ?? []).Union(def.equippedStatOffsets ?? []).Any(sm =>
                _statWeights.Any(statDef => statDef.StatDef == sm.stat))));
    ...
    public float GetThingScore([NotNull] Thing thing)
    {
        return thing == null
            ...
            : _statWeights.Where(statWeight => statWeight.StatDef != null).Sum(statWeight =>
                EquipmentManager.NormalizeStatValue(statWeight.StatDef,
                    StatHelper.GetStatValueDeviation(thing, statWeight.StatDef)) *
                statWeight.Weight);
    }
    ...
    private void Initialize()
    {
                     ...
                     recipeDef.requiredGiverWorkType == WorkTypeDef))
        {
            ...
            if (recipe.workTableEfficiencyStat != null && !_defaultStatWeights.Any(sw =>
