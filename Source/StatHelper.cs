...
internal static class StatHelper
{
    ...
    private static IEnumerable<StatDef> CustomStatsDefs =>
        CustomMeleeWeaponStats.StatDefs.Union(CustomRangedWeaponStats.StatDefs)
    ...
    private static IEnumerable<StatDef> DefaultWeaponStatDefs =>
        DefaultStatDefs.Where(def =>
            WeaponCategories.Contains(def.category?.defName ?? string.Empty));

    public static IReadOnlyList<StatDef> MeleeWeaponStatDefs { get; } = new List<StatDef>(
        CustomMeleeWeaponStats.StatDefs.Union(DefaultWeaponStatDefs)
            .OrderBy(def => def.category?.defName ?? string.Empty).ThenBy(def => def.label));

    [NotNull]
    private static IEnumerable<string> PawnCategories =>
    [
        "Basics", "BasicsImportant", "BasicsPawnImportant", "BasicsPawn", "PawnCombat",
        "PawnSocial",
        "PawnMisc", "PawnWork"
    ];

    public static IReadOnlyList<StatDef> RangedWeaponStatDefs { get; } = new List<StatDef>(
        CustomRangedWeaponStats.StatDefs.Union(DefaultWeaponStatDefs)
            .OrderBy(def => def.category?.defName ?? string.Empty).ThenBy(def => def.label));

    public static IReadOnlyList<StatDef> ToolStatDefs { get; } = new List<StatDef>(CustomToolStats
    ...
    [
        "Basics", "BasicsImportant", "BasicsNonPawnImportant", "BasicsNonPawn", "Weapon",
        "Weapon_Ranged",
        "Weapon_Melee", "PawnWork"
    ];

    [NotNull] private static IEnumerable<string> WorkCategories => ["PawnWork", "PawnSocial"];
    ...
    public static float GetStatValue([NotNull] Thing thing, [NotNull] StatDef statDef)
    {
        ...
        if (IsStatDisabled(thing, statDef)) { return 0f; }
        try
        {
            return thing.GetStatValue(statDef) + (thing.def.equippedStatOffsets
                ?.Find(modifier => modifier.stat == statDef)?.value ?? 0f);
        }
    ...
    public static bool IsStatDisabled([NotNull] Thing thing, [NotNull] StatDef statDef)
    {
        if (thing == null) { throw new ArgumentNullException(nameof(thing)); }
        if (statDef == null) { throw new ArgumentNullException(nameof(statDef)); }
        return statDef.Worker?.IsDisabledFor(thing) ?? false;
    }
