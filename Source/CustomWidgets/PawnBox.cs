...
{
    public static void DoPawnBox(Rect rect, Color backgroundColor, Color outlineColor,
        int columnCount, float entryGap, ref Vector2 scrollPosition,
        [NotNull] IReadOnlyList<Pawn> pawns)
    {
        ...
        var entryWidth =
            (-1 * columnCount * entryGap + entryGap + (rect.width - horizontalMargin)) /
            columnCount;
        ...
        for (var i = 0; i < pawns.Count; i++)
        {
            ...
            if (Event.current.type == EventType.MouseDown && Event.current.button == 0 &&
                Mouse.IsOver(entryRect)) { Find.WindowStack.Add(new Dialog_InfoCard(pawn)); }
        }
