...
{
    public static void DoThingBox(Rect rect, Color backgroundColor, Color outlineColor,
        float iconSize, float iconGap, ref Vector2 scrollPosition,
        [NotNull] IReadOnlyList<Thing> things, Action<Thing> rightClickAction,
        Func<Thing, string> tooltipGetter)
    {
        ...
        for (var i = 0; i < things.Count; i++)
        {
            ...
            var texture = thing.StyleDef != null && thing.StyleDef.UIIcon != null
                ?
                thing.StyleDef.UIIcon
                : !thing.def.uiIconPath.NullOrEmpty()
                    ? thing.def.uiIcon
    ...
    public static void DoThingDefBox(Rect rect, Color backgroundColor, Color outlineColor,
        float iconSize, float iconGap, ref Vector2 scrollPosition,
    ...
    private static Rect GetThingRect(Rect rect, float iconSize, float iconGap, int columnCount,
        int index)
    {
        var rowIndex = Math.DivRem(index, columnCount, out var columnIndex);
        return new Rect(rect.x + (iconSize + iconGap) * columnIndex,
            rect.y + (iconSize + iconGap) * rowIndex, iconSize, iconSize);
    }
}